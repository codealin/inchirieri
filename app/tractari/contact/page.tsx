import type { Metadata } from 'next'
import { Building2, MapPin, Phone, Clock } from 'lucide-react'
import { BUSINESS, SITE_URL } from '@/lib/config'

export const metadata: Metadata = {
  title: 'Contact — Tractări Auto',
  description: `Contactează ${BUSINESS.legalName} pentru servicii de tractări auto în Alba Iulia. Telefon ${BUSINESS.phoneDisplay}.`,
  alternates: { canonical: `${SITE_URL}/tractari/contact` },
}

export default function ContactPage() {
  return (
    <section className="py-16 px-4">
      <div className="max-w-6xl mx-auto">
        <p className="font-medium mb-2 text-sm tracking-wide uppercase" style={{ color: '#16a34a' }}>
          Contact
        </p>
        <h1 className="text-4xl md:text-5xl font-bold mb-10">Datele companiei</h1>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          {/* Company info */}
          <div className="space-y-1 divide-y divide-slate-100 border rounded-2xl overflow-hidden bg-white shadow-sm">
            <Row icon={Building2}>
              <div className="grid grid-cols-2 gap-6 w-full">
                <div>
                  <p className="text-sm text-slate-500 mb-0.5">Nume companie</p>
                  <p className="font-semibold">{BUSINESS.legalName}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-500 mb-0.5">CUI</p>
                  <p className="font-semibold">{BUSINESS.cui}</p>
                </div>
              </div>
            </Row>

            <Row icon={MapPin}>
              <div>
                <p className="text-sm text-slate-500 mb-0.5">Adresă</p>
                <p className="font-semibold">{BUSINESS.fullAddress}</p>
              </div>
            </Row>

            <Row icon={Phone}>
              <div className="flex items-center justify-between gap-4 w-full flex-wrap">
                <div>
                  <p className="text-sm text-slate-500 mb-0.5">Telefon</p>
                  <a
                    href={`tel:${BUSINESS.phone}`}
                    className="font-semibold hover:underline"
                    style={{ color: '#16a34a' }}
                  >
                    {BUSINESS.phoneDisplay}
                  </a>
                </div>
                <a
                  href={BUSINESS.whatsapp}
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ backgroundColor: '#25d366', color: '#fff' }}
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-opacity hover:opacity-90"
                >
                  <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
                  WhatsApp
                </a>
              </div>
            </Row>

            <Row icon={Clock}>
              <div>
                <p className="text-sm text-slate-500 mb-0.5">Program de lucru</p>
                <p className="font-semibold">
                  Luni – Duminică:{' '}
                  <span style={{ color: '#16a34a' }}>Deschis Non-Stop</span>
                </p>
              </div>
            </Row>
          </div>

          {/* Map */}
          <div>
            <p className="text-sm text-slate-500 mb-2">Locația companiei</p>
            <p className="font-semibold mb-3">{BUSINESS.legalName}</p>
            <div className="rounded-2xl overflow-hidden border shadow-sm h-80">
              <iframe
                src="https://www.google.com/maps?q=Expert+Doi+Trans+Micești+Alba+Iulia&output=embed"
                width="100%"
                height="100%"
                style={{ border: 0 }}
                allowFullScreen
                loading="lazy"
                referrerPolicy="no-referrer-when-downgrade"
                title="Locație Expert Doi Trans"
              />
            </div>
            <a
              href="https://maps.app.goo.gl/cY2PnrWXhNDqAvJXA"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block mt-3 text-sm font-medium hover:underline"
              style={{ color: '#16a34a' }}
            >
              Deschide în Google Maps →
            </a>
          </div>
        </div>
      </div>
    </section>
  )
}

function Row({ icon: Icon, children }: { icon: React.ElementType; children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-4 p-5">
      <div
        className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0"
        style={{ backgroundColor: '#f0fdf4' }}
      >
        <Icon className="h-5 w-5" style={{ color: '#16a34a' }} />
      </div>
      <div className="flex-1 min-w-0">{children}</div>
    </div>
  )
}
